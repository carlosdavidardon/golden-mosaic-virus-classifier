...
#It uses features extraction from an intermediate layer
...
def create_model():
	if FLAGS.arch == 'resnet':
		...
		base_model = tf.keras.models.Model(inputs=base_model.input, 
											outputs=base_model.get_layer('conv5_block1_preact_relu').output)
		...
		base_model = tf.keras.models.Model(inputs=base_model.input, 
											outputs=base_model.get_layer('block_14_project_BN').output)
	else:
